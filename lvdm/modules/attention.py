...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, n_heads, d_head, dropout=0., context_dim=None, gated_ff=True, checkpoint=True,
        ...
        self.attn2 = attn_cls(
            query_dim=dim, context_dim=context_dim, heads=n_heads, dim_head=d_head, dropout=dropout, video_length=video_length,
            image_cross_attention=image_cross_attention, image_cross_attention_scale=image_cross_attention_scale,
...
class TemporalTransformer(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head, depth=1, dropout=0., context_dim=None,
        ...
        if relative_position:
            assert(temporal_length is not None)
            attention_cls = partial(CrossAttention, relative_position=True, temporal_length=temporal_length, is_temporal=True)
        else:
            attention_cls = partial(CrossAttention, temporal_length=temporal_length, is_temporal=True)
        ...
        self.transformer_blocks = nn.ModuleList([
            BasicTransformerBlock(
                ...
                attention_cls=attention_cls,
                checkpoint=use_checkpoint,
                is_temporal=True) for d in range(depth)
        ])
        if not use_linear:
