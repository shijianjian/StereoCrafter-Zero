...
import copy
from lvdm.models.samplers.stereo_utils import stereo_shift_torch
...
class DDIMSampler(object):
    ...
    def ddim_sampling(self, cond, shape,
        ...
        # assert False, (torch.as_tensor(alphas_prev).cpu())
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
                      temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None,
                      unconditional_guidance_scale=1., unconditional_conditioning=None, is_duplicate=False, add_noise=True,
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            ...
            #     model_output_cp = rescale_noise_cfg(model_output, e_t_cond, guidance_rescale=guidance_rescale)
            #     if is_duplicate:
            #         model_output_cp[1:2] = rescale_noise_cfg(model_output, e_t_cond, guidance_rescale=0.)[1:2]
            #     model_output = model_output_cp
        ...
        if is_duplicate:
            disparity = kwargs.get("disparity", None)
            # noise[x.size(0) // 2:] = noise[:x.size(0) // 2]
            # disparity = None
            if disparity is not None:
                # noise_2 = sigma_t * noise_like(x.shape, device, repeat=True) * temperature
                disparity = torch.nn.functional.interpolate(
                    disparity[0],
                    size=[noise.size(-2), noise.size(-1)],
                    mode="bicubic",
                    align_corners=False
                )[None]
                latents = noise
                _, c, t, h, w = latents.shape
                # disp = disparity[0, 7:8].repeat(16, 1, 1, 1)
                disp = disparity[0]
                # uncertainty_mask = compute_uncertainty_mask(disp, threshold=0.3).bool()
                # print("uncertainty_mask", uncertainty_mask.sum() / torch.ones_like(uncertainty_mask).sum())
                # disp[uncertainty_mask] = 0
                latents_ts = stereo_shift_torch(
                    latents[0], disp[:, 0], scale_factor=kwargs.get("stereo_scale_factor", None), shift_both=False)
                latent_l = latents_ts[:1]
                ...
                # Spatial inpainting
                # filling_latents_sp = torch.stack([
                #     inpainting.forward(latent_r[0, :, j], mask_r[0, :, j]) for j in range(t)
                # ], dim=1)[None]
                # # temporal inpainting
                # filling_latents_tp = torch.stack([
                #     inpainting.forward(latent_r[0, j], mask_r[0, j]) for j in range(c)
                # ], dim=0)[None]
                # filling_latents = (filling_latents_sp + filling_latents_tp) / 2
                # latent_r[~mask_r] = filling_latents_sp[~mask_r]
                latent_r[~mask_r] = noise[x.size(0) // 2:][~mask_r]
                # n_mean = noise.mean(dim=(2, 3), keepdim=True)[:1]
                # noise_2 = (noise[:1] - n_mean) *.8 + n_mean
                # latent_r[~mask_r] = (noise_2[0:1] * 0.5 + latent_l * 0.5)[~mask_r]
                noise[x.size(0) // 2:] = latent_r
            else:
                # noise[x.size(0) // 2:] = torch.zeros_like(noise[:x.size(0) // 2])
                noise[x.size(0) // 2:] = noise[:x.size(0) // 2]
        if noise_dropout > 0.:
        ...
        output_all = kwargs.get("output_all", None)
        if output_all:
            return x_prev, pred_x0, a_prev.sqrt(), dir_xt, noise
